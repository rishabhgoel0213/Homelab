...
let
  ...
  quarto = pkgs.callPackage ./quarto-package.nix { };
  ...
  blogctl = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      exec ${python}/bin/python3 ${./bin/blog-admin.py} "$@"
    '';
  };
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.blog-admin = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${blogctl}/bin/blogctl serve --assets ${./assets} --editor-url ${editorUrl} --public-url ${publicUrl} --preview-url ${previewUrl} --port ${toString cfg.adminPort}";
        Restart = "on-failure";
        RestartSec = "5s";
        UMask = "0022";
