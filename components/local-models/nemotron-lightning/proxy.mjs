...
const listenHost = process.env.NEMOTRON_HOST ?? "127.0.0.1";
const listenPort = Number(process.env.NEMOTRON_PORT ?? 8003);
const backendHost = "127.0.0.1";
const backendPort = Number(process.env.NEMOTRON_BACKEND_PORT ?? 8004);
const servedModel = process.env.NEMOTRON_MODEL ?? "nemotron-3.5-lightning-30b-a3b";
...
function normalizeReasoning(body) {
  ...
  const effort = requested == null ? "none" : String(requested).toLowerCase();
...
const server = createServer(async (request, response) => {
  try {
    if (request.method === "POST" && request.url?.startsWith("/v1/chat/completions")) {
      ...
      const enableThinking = normalizeReasoning(body);
