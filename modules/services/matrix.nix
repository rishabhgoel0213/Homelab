...
let
  ...
  elementWebConfig = pkgs.writeText "element-web-config.json" (
    builtins.toJSON {
      default_server_config."m.homeserver".base_url = "https://${clientHost}";
      disable_custom_urls = true;
      disable_guests = true;
      default_federate = false;
      default_country_code = "US";
      default_device_display_name = "Homelab Web";
      login_for_welcome = true;
      permalink_prefix = "https://${webHost}";
      room_directory.servers = [ ];
      show_labs_settings = false;
      setting_defaults."UIFeature.urlPreviews" = false;
    }
  );
  elementWeb = pkgs.runCommand "element-web-homelab-${pkgs.element-web.version}" { } ''
    cp -R ${pkgs.element-web}/. "$out"
    chmod -R u+w "$out"
    cp ${elementWebConfig} "$out/config.json"
    cp ${elementWebConfig} "$out/config.${webHost}.json"
  '';
...
in
{
  config = lib.mkIf cfg.matrix.enable {
    ...
    homelab.routes.chat = {
      ...
      host = "chat";
      visibility = "internal";
      root = toString elementWeb;
      description = "Private Element Web Matrix client";
      extraConfig = ''
        @elementRuntimeConfig path / /index.html /config.json /config.*.json /manifest.json /service-worker.js
        header @elementRuntimeConfig Cache-Control "no-store"
        header {
